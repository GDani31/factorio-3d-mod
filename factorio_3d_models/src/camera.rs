...
//   (mouse steers; hold ctrl or esc to free the cursor; scroll out exits)
// - altgr (right alt): reset the view to top-down vanilla
...
pub fn init() {
    *CAMERA.lock().unwrap() = Some(Camera::default());
    log::info!("[camera] ready (shift+rmb / mmb rotates, shift+scroll zooms, altgr resets)");
}
...
unsafe extern "system" fn scroll_wndproc(
    ...
    if msg == WM_MOUSEWHEEL {
        ...
        // in first person the wheel belongs to the mod alone (scroll-out
        // exits; the game must not zoom underneath)
...
// virtual-key codes for key_held
const VK_SHIFT: i32 = 0x10;
const VK_RBUTTON: i32 = 0x02;
const VK_MBUTTON: i32 = 0x04;
const VK_RMENU: i32 = 0xA5; // right alt (altgr)
...
pub fn poll() {
    ...
    let shift = key_held(VK_SHIFT);
    let rmb = key_held(VK_RBUTTON);
    let mmb = key_held(VK_MBUTTON);
    ...
    // so one press resets once)
    let altgr = key_held(VK_RMENU);
    ...
    if scroll != 0 && shift {
        if let Some(cam) = &mut *CAMERA.lock().unwrap() {
            if scroll > 0 && cam.zoom <= 1.001 {
                ...
                log::info!(
                    "[camera] first-person ON (ctrl/esc frees the cursor, scroll out exits)"
                );
            } else {
...
// holding ctrl or esc frees the cursor (esc = while the game menu is open)
fn poll_first_person() {
    MOUSE_TRACKING.store(false, Ordering::Relaxed);
    let unlock = key_held(VK_CONTROL) || key_held(VK_ESCAPE);
    let hwnd = game_hwnd();
    let foreground =
        unsafe { windows::Win32::UI::WindowsAndMessaging::GetForegroundWindow() == hwnd };

    if !unlock && foreground && !hwnd.0.is_null() {
        unsafe {
