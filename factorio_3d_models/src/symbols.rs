    ...
    while let Some(symbol) = iter.next()? {
        if let Ok(pdb::SymbolData::Public(data)) = symbol.parse() {
            ...
            let wanted = crate::offsets::ALL
                ...
                || crate::offsets::GENERIC_DRAWS.iter().any(|d| {
                    (!d.gf.symbol.is_empty() && name.contains(d.gf.symbol))
                        || (!d.dir_sym.is_empty() && name.contains(d.dir_sym))
                        || (!d.orient_sym.is_empty() && name.contains(d.orient_sym))
                        || (!d.turret_sym.is_empty() && name.contains(d.turret_sym))
                        || (!d.progress_sym.is_empty() && name.contains(d.progress_sym))
                        || (!d.activity_sym.is_empty() && name.contains(d.activity_sym))
                });
            if wanted {
