...
// (or `inject.exe [path\to\factorio_3d_models.dll]`).
...
fn main() -> Result<()> {
    println!("=== factorio_3d_models injector ===");
...
fn find_dll() -> Result<PathBuf> {
    ...
    let exe_dir = std::env::current_exe()?
        ...
        .to_path_buf();
    let candidates = [
        exe_dir.join("factorio_3d_models.dll"),
        PathBuf::from("target/release/factorio_3d_models.dll"),
        PathBuf::from("target/debug/factorio_3d_models.dll"),
    ];
    for c in &candidates {
        if c.exists() {
    ...
    anyhow::bail!("factorio_3d_models.dll not found — build it first: cargo build --release")
}
